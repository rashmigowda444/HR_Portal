...
<button onclick="myFunction()" id="but">Download PDF</button><br><br>
...
WHERE pay.emp_id ='$id' and YEAR(pay.date_generated)='$year' and MONTH(pay.date_generated)='$month' limit 1";
...
$time=strtotime($row['date_generated']);
   ...
   $gross= $row['professional_tax']+$row['income_tax'];
...
echo"
   ...
   <h4 style='font-family:Helvetica;'>PaySlip for Month {$month} {$year}</h4>
   </div>
   </div><br><hr>
   ...
   <div class='col-md-6 col-sm-6 col-lg-6'style='float:left;width:50%;'>
    <table class='table table-bordered'>
    <thead>
      <tr>
        <th>DEDUCTIONS</th>
...
echo'<script>alert("No data found for selected month and year")</script>';
